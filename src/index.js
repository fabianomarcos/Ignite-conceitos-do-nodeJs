...
app.post('/users', (request, response) => {
  ...
  const userAlready = users.find((userFilter) => userFilter.username === username);

  if (userAlready) return response.status(400).json({ error: 'Username already exists!' });
...
app.put('/todos/:id', checksExistsUserAccount, (request, response) => {
  ...
  const todo = user.todos.find((todoF) => todoF.id === id);
